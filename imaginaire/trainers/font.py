...
class Trainer(BaseTrainer):
    ...
    def __init__(self, cfg, net_G, net_D, opt_G, opt_D, sch_G, sch_D,
        ...
        super().__init__(cfg, net_G, net_D, opt_G, opt_D, sch_G, sch_D,
                         train_data_loader, val_data_loader)
        self._init_feat_extractor()

    def _init_feat_extractor(self):
        model = models.resnet18()
        model.conv1 = nn.Conv2d(1, 64, kernel_size=(7, 7), stride=(2, 2), padding=(3, 3), bias=False)
        model.fc = nn.Linear(in_features=512, out_features=3499, bias=True)
        state_dict = torch.load('char_featex.pth')
        model.load_state_dict(state_dict['model'])
        self.char_featex = model.cuda().eval()
        for p in self.char_featex.parameters():
    ...
    def gen_forward(self, data):
        ...
        self.gen_losses['image_recon'] = \
            self.criteria['image_recon'](net_G_output['images_recon'],
                                         data['images_content'])
        # Recognizability loss by image space matching
        self.gen_losses['image_recon'] += \
            0.5 * self.criteria['image_recon'](net_G_output['images_trans'],
                                         data['images_content'])
        ...
        # Recognizability loss by feature space matching
        content_feat = get_feat(self.char_featex, data['images_content'])
        output_feat = get_feat(self.char_featex, net_G_output['images_trans'])
        self.gen_losses['feature_matching'] += \
            1 * self.criteria['feature_matching'](content_feat, output_feat)
